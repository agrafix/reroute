...
instance Functor PathMap where
  fmap f (PathMap sc h s p) = PathMap (fmap (fmap f) sc) (fmap f h) (fmap (fmap f) s) (fmap f p)

instance NFData x => NFData (PathMap x) where
  rnf (PathMap sc h s p) = rnf sc `seq` rnf h `seq` rnf s `seq` PM.rnfHelper rnf p
...
instance Monoid (PathMap x) where
  mempty = emptyPathMap
  mappend (PathMap sc1 h1 s1 p1) (PathMap sc2 h2 s2 p2) =
    PathMap (sc1 `mappend` sc2) (h1 `mappend` h2)
            (HM.unionWith mappend s1 s2) (PM.unionWith mappend p1 p2)

insertPathMap' :: Path ts -> (HVect ts -> x) -> PathMap x -> PathMap x
insertPathMap' path action (PathMap sc h s p) =
  case path of
    Empty -> PathMap sc (action HNil : h) s p
    StaticCons pathPiece path' ->
      let subPathMap = fromMaybe emptyPathMap (HM.lookup pathPiece s)
      in PathMap sc h (HM.insert pathPiece (insertPathMap' path' action subPathMap) s) p
    VarCons path' ->
      let alterFn = Just . insertPathMap' path' (\vs v -> action (HCons v vs))
                         . fromMaybe emptyPathMap
      in PathMap sc h s (PM.alter alterFn p)
...
insertSubComponent' path comp (PathMap sc h s p) =
  case path of
    Empty -> PathMap (comp HNil : sc) h s p
    StaticCons pathPiece path' ->
      let subPathMap = fromMaybe emptyPathMap (HM.lookup pathPiece s)
      in PathMap sc h (HM.insert pathPiece (insertSubComponent' path' comp subPathMap) s) p
    VarCons path' ->
      let alterFn = Just . insertSubComponent' path' (\vs ps v -> comp (HCons v vs) ps)
                         . fromMaybe emptyPathMap
      in PathMap sc h s (PM.alter alterFn p)

insertSubComponent :: Functor m => RouteHandle m ([T.Text] -> a) -> PathMap (m a) -> PathMap (m a)
insertSubComponent (RouteHandle path comp) =
  insertSubComponent' path (fmap (\m ps -> fmap ($ ps) m) (hVectUncurry comp))

match :: PathMap x -> [T.Text] -> [x]
match (PathMap sc h s p) pieces =
  map ($ pieces) sc ++
  case pieces of
    [] -> h
    (pp:pps) ->
