...
defSubComponent :: (Monad m, Functor m, m ([T.Text] -> ReturnVar) ~ x)
                 => Path ts
                 -> HVectElim ts x
                 -> RegistryT (SafeRouter m ReturnVar) middleware Bool m ()
defSubComponent path comp =
    do let reqType = True
       SafeRouterPath basePath <- ask
       modify $ \rs ->
           rs { rs_registry =
                    let SafeRouterReg (reg, fb) = fromMaybe emptyRegistry (HM.lookup reqType (rs_registry rs))
                        reg' = insertSubComponent (RouteHandle (basePath </> path) comp) reg
                    in HM.insert reqType (SafeRouterReg (reg', fb)) (rs_registry rs)
              }
